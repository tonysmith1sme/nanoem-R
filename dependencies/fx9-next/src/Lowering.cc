    ...
    DiagnosticSink &diagnostics) const
{
    ...
    for (std::vector<EffectBindingIR>::iterator binding = effect.bindings.begin(); binding != effect.bindings.end(); ++binding) {
        if (binding->registerSet == kEffectRegisterSampler && binding->textureName.empty()) {
            std::unordered_map<std::string, std::string>::const_iterator it = sampleRelationships.find(binding->name);
            if (it != sampleRelationships.end()) {
                binding->textureName = it->second;
            }
        }
    }
    for (std::vector<Variable>::const_iterator variable = unit.variables.begin(); variable != unit.variables.end();
        ...
        std::string textureName = variable->textureName;
        if (textureName.empty()) {
            const EffectBindingIR *binding = nullptr;
            for (std::vector<EffectBindingIR>::const_iterator it = effect.bindings.begin(); it != effect.bindings.end(); ++it) {
                if (it->name == variable->name && it->registerSet == kEffectRegisterSampler) {
                    binding = &*it;
                    break;
                }
            }
        ...
        if (!found) {
            diagnostics.add(kDiagnosticEffect, kDiagnosticError, "FX9T1005", SourceLocation(), variable->name,
                "sampler texture does not resolve: " + textureName);
            return false;
        }
    }
