...
#pragma once
#ifndef FX9_EFFECT_PIPELINE_H_
#define FX9_EFFECT_PIPELINE_H_
...
/**
 * Modular MME effect compilation pipeline.
 *
 * Replaces the former monolithic Compiler::compile workflow with staged
 * ownership:
 *   1. Source loading / text decode
 *   2. EffectSourcePipeline preparation
 *   3. Frontend technique discovery (Lemon + ParserContext)
 *   4. Per-pass SPIR-V generation + backend cross-translate
 *   5. Parameter / annotation / include protobuf emission
 *
 * Public entry remains Compiler (thin facade). Lemon grammar, glslang HLSL
 * AST, SPIR-V, SPIRV-Cross, and protobuf ABI are preserved.
 */
...
public:
    ...
    EffectPipeline(EProfile profile, EShMessages messages);
    ~EffectPipeline();
...
private:
    ...
    struct BaseParameterConverter {
        ...
        EffectPipeline *m_parent = nullptr;
        void *m_opaque = nullptr;
    };

    /* ---- per-pass backend (SPIR-V + cross-translate) ---- */
    struct BasePassShader {
        BasePassShader(EffectPipeline *parent, const char *path, const glslang::TString &source,
    ...
    struct GLSLPassShader : BasePassShader {
        GLSLPassShader(EffectPipeline *parent, const char *path, const glslang::TString &source,
            EffectProduct &effectProduct, void *opaque);
    ...
    struct MSLPassShader : BasePassShader {
        MSLPassShader(EffectPipeline *parent, const char *path, const glslang::TString &source,
            EffectProduct &effectProduct, void *opaque);
    ...
    struct HLSLPassShader : BasePassShader {
        HLSLPassShader(EffectPipeline *parent, const char *path, const glslang::TString &source,
            EffectProduct &effectProduct, void *opaque);
    ...
    struct SPIRVPassShader : BasePassShader {
        SPIRVPassShader(EffectPipeline *parent, const char *path, const glslang::TString &source,
            EffectProduct &effectProduct, void *opaque);
