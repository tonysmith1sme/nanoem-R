...
function(compile_ffmpeg _cmake_build_type _generator _toolset_option _arch_option _triple_path)
  ...
  if(DEFINED ENV{FFMPEG_VERSION_TAG})
    set(_branch_name $ENV{FFMPEG_VERSION_TAG})
  else()
    set(_branch_name "n8.1.2")
  endif()
  ...
  list(APPEND _ffmpeg_build_options "--enable-demuxer=avi,matroska,mov,mp3,wav")
  list(APPEND _ffmpeg_build_options "--enable-muxer=avi,flac,matroska,mov,mp4,webm,wav")
  list(APPEND _ffmpeg_build_options "--enable-decoder=aac,flac,h264,hevc,hq_hqa,hqx,mjpeg,mp3,pcm_s16le,prores,rawvideo,utvideo,vp8,vp9")
  set(_ffmpeg_encoders "aac,ffv1,flac,mjpeg,mpeg4,pcm_s16le,prores,prores_ks,rawvideo,utvideo")
  if(APPLE)
    string(APPEND _ffmpeg_encoders ",h264_videotoolbox,hevc_videotoolbox,vp8_videotoolbox,vp9_videotoolbox")
  elseif(WIN32)
    string(APPEND _ffmpeg_encoders ",h264_mf,hevc_mf")
  endif()
  list(APPEND _ffmpeg_build_options "--enable-encoder=${_ffmpeg_encoders}")
  list(APPEND _ffmpeg_build_options "--enable-parser=aac,h264,hevc,mjpeg,mpeg4video,vp8,vp9")
  list(APPEND _ffmpeg_build_options "--enable-bsf=aac_adtstoasc,h264_mp4toannexb,hevc_mp4toannexb,vp9_superframe")
  list(APPEND _ffmpeg_build_options "--enable-protocol=file")
  list(APPEND _ffmpeg_build_options "--enable-cross-compile")
  if(APPLE)
    list(APPEND _ffmpeg_build_options "--enable-videotoolbox")
  elseif(WIN32)
    list(APPEND _ffmpeg_build_options "--enable-mediafoundation")
  endif()
  execute_process(COMMAND ${GIT_EXECUTABLE} checkout ${_branch_name} WORKING_DIRECTORY ${_source_path})
  if(APPLE)
    if("${_arch}" STREQUAL "ub")
    ...
    if("${_arch}" STREQUAL "ub")
      ...
      file(GLOB _avcodec "${_interm_arm64_path}/lib/libavcodec.*.dylib")
      file(GLOB _avformat "${_interm_arm64_path}/lib/libavformat.*.dylib")
      file(GLOB _avutil "${_interm_arm64_path}/lib/libavutil.*.dylib")
      file(GLOB _swresample "${_interm_arm64_path}/lib/libswresample.*.dylib")
      file(GLOB _swscale "${_interm_arm64_path}/lib/libswscale.*.dylib")
      list(FILTER _avcodec EXCLUDE REGEX "\\.[0-9]+\\.[0-9]+\\.[0-9]+\\.dylib$")
      list(FILTER _avformat EXCLUDE REGEX "\\.[0-9]+\\.[0-9]+\\.[0-9]+\\.dylib$")
      list(FILTER _avutil EXCLUDE REGEX "\\.[0-9]+\\.[0-9]+\\.[0-9]+\\.dylib$")
      list(FILTER _swresample EXCLUDE REGEX "\\.[0-9]+\\.[0-9]+\\.[0-9]+\\.dylib$")
      list(FILTER _swscale EXCLUDE REGEX "\\.[0-9]+\\.[0-9]+\\.[0-9]+\\.dylib$")
      list(GET _avcodec 0 _avcodec_path)
      list(GET _avformat 0 _avformat_path)
      list(GET _avutil 0 _avutil_path)
      list(GET _swresample 0 _swresample_path)
      list(GET _swscale 0 _swscale_path)
      get_filename_component(_avcodec ${_avcodec_path} NAME)
      get_filename_component(_avformat ${_avformat_path} NAME)
      get_filename_component(_avutil ${_avutil_path} NAME)
      get_filename_component(_swresample ${_swresample_path} NAME)
      get_filename_component(_swscale ${_swscale_path} NAME)
